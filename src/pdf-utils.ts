...
export async function loadTestPdfText(): Promise<string> {
	const docProxy: pdfjsTypes.PDFDocumentProxy = await pdfjsLib.getDocument(TEST_PDF_FILENAME).promise
	return pdfProxy2Str(docProxy)
}

export async function pdfProxy2Str(pdfProxy: pdfjsTypes.PDFDocumentProxy): Promise<string> {
	let res = ''
...
export async function file2PdfProxy(file: File): Promise<pdfjsTypes.PDFDocumentProxy> {
